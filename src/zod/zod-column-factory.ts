...
import { BUFFER_SCHEMA_ZOD, JSON_SCHEMA_ZOD } from "./constants";

export function getDefaultZodColumnFactoryRegistry(factory: SchemaFactory<z.ZodType>) {
  const columnTypeRegistry = new ColumnFactoryRegistry<z.ZodType>();
  ...
  columnTypeRegistry.register(["PgHalfVector", "PgVector"], (column: PgHalfVector<any>) => z.array(z.number()).length(column.dimensions));
  ...
  columnTypeRegistry.register(["PgArray"], (column: PgArray<any, any>) =>
    z.array(factory.columnToBaseSchema(column.baseColumn)).length(column.size),
  );
  columnTypeRegistry.register(["PgUUID"], () => z.uuid());
  columnTypeRegistry.register(BINARY_VECTOR_COLUMN_TYPES, (column: BinaryVector) =>
    z.string().regex(BINARY_VECTOR_REGEX).max(column.dimensions),
  );
  ...
  // String types
  columnTypeRegistry.register(VARCHAR_COLUMN_TYPES, (column: Varchar) => z.string().max(column.length ?? CONSTANTS.INT16_UNSIGNED_MAX));
  ...
  // Other types
  columnTypeRegistry.register(["json"], (_: Column) => JSON_SCHEMA_ZOD);
  columnTypeRegistry.register(["buffer"], (_: Column) => BUFFER_SCHEMA_ZOD);
  ...
  columnTypeRegistry.register(["enum"], (column: Column) => {
    return z.enum(column.enumValues ?? []);
  });
