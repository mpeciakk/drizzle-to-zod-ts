...
import { customType, int, mysqlSchema, mysqlTable, mysqlView, serial, text, varchar } from "drizzle-orm/mysql-core";
...
import { expectSchemaShape } from "../common";
...
const zodSchemaFactory = new ZodSchemaFactory();
...
test("table - select", () => {
  ...
  const result = createSelectSchema(table, zodSchemaFactory) as z.ZodObject;
  const expected = z.object({ id: serialNumberModeSchema, generated: intSchema, name: textSchema });
  expectSchemaShape(expected).from(result);
});
...
//     name: z.string(),
//     age: z.string(), // not optional
//     tags: z.array(z.string()),
//   });

//   expectSchemaShape(schemaA).from(schemaB);
//   expectSchemaShape(schemaA).from(schemaC);
// });
...
test("table - insert", () => {
  ...
  const result = createInsertSchema(table, zodSchemaFactory) as z.ZodObject;
  const expected = z.object({
    id: serialNumberModeSchema.optional(),
    name: z.string().max(CONSTANTS.INT16_UNSIGNED_MAX),
    age: intSchema.optional().nullable(),
  });
  expectSchemaShape(expected).from(result);
});
...
test("table - update", () => {
  ...
  const result = createUpdateSchema(table, zodSchemaFactory) as z.ZodObject;
  const expected = z.object({
    id: serialNumberModeSchema.optional(),
...
test("view qb - select", () => {
  ...
  const result = createSelectSchema(view, zodSchemaFactory) as z.ZodObject;
...
test("nullability - insert", () => {
  ...
  const expected = z.object({
    ...
    c2: intSchema,
    c3: intSchema.nullable().optional().default(1),
    c4: intSchema.optional().default(1),
  });
  expectSchemaShape(expected).from(result);
});
...
test("refine table - select", () => {
  ...
  const result = createSelectSchema(table, zodSchemaFactory, {
    c2: (schema) => (schema as z.ZodNumber).lte(1000),
...
test("refine table - insert", () => {
  const table = mysqlTable("test", {
    ...
    c5: varchar({ length: 10 }).notNull(),
  });

  const result = createInsertSchema(table, zodSchemaFactory, {
    c2: (schema) => (schema as z.ZodNumber).lte(1000),
    c3: z.string().transform(Number),
    c5: (schema: z.ZodType) => schema,
  }) as z.ZodObject;
...
test("refine table - update", () => {
  ...
  const result = createUpdateSchema(table, zodSchemaFactory, {
    c2: (schema) => (schema as z.ZodNumber).lte(1000),
  }) as z.ZodObject;
  const expected = z.object({
    c1: intSchema.nullable().optional(),
...
test("refine view - select", () => {
  ...
  const result = createSelectSchema(view, zodSchemaFactory, {
    c2: (schema) => (schema as z.ZodNumber).lte(1000),
    nested: {
      c5: (schema) => (schema as z.ZodNumber).lte(1000),
    },
    table: {
      c2: (schema) => (schema as z.ZodNumber).lte(1000),
    },
